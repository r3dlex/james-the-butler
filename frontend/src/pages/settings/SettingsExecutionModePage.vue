<template>
  <div class="p-6">
    <h1 class="mb-4 text-lg font-medium" style="color: var(--color-text)">Execution Mode</h1>
    <div class="max-w-lg space-y-4">
      <div>
        <label class="mb-1 block text-xs font-medium" style="color: var(--color-text-dim)">Account Default</label>
        <select
          v-model="mode"
        ...
        >
          <option value="direct">Direct — agents execute immediately</option>
          <option value="confirmed">Confirmed — destructive actions require approval</option>
        </select>
      </div>
      <p class="text-xs" style="color: var(--color-text-dim)">
        Projects and sessions can override this setting. The most specific level wins.
      </p>
      <button
